            ...
            <div class="listing-data">
                <div class="listing-basicinfo">
                    ...
                    <div class="listing-attributes">
                        <span class="category"><i class="fa fa-th-large font-awe-cat" aria-hidden="true"></i> <?php echo osc_item_category() ; ?></span>
                        <span class="location"><i class="fa fa-map-marker" aria-hidden="true"></i> <?php echo osc_item_city(); ?> <?php if( osc_item_region()!='' ) { ?> (<?php echo osc_item_region(); ?>)<?php } ?></span> <span class="g-hide">-</span><i class="fa fa-calendar" aria-hidden="true"></i> <?php echo osc_format_date(osc_item_pub_date()); ?>
                        <?php if(osc_count_item_resources()>0) { ?>
            <span class="listing-thumb-count-postion"> <i class="fa fa-camera"></i> [<span  class="listing-thumb-count text-center"><?php echo osc_count_item_resources(); ?></span>]</span>
            <?php } ?>
                        <?php if( osc_price_enabled_at_items() ) { ?><span class="currency-value"> <?php echo osc_format_price(osc_item_price()); ?></span><?php } ?>
                    </div>
                    <p class="show_desc"><?php echo osc_highlight( osc_item_description() ,235) ; ?></p>
                </div>
                <?php if($admin){ ?>
                    <span class="admin-options">
                        <a href="<?php echo osc_item_edit_url(); ?>" rel="nofollow"><?php _e('Edit item', 'passion'); ?></a>
                        <span>|</span>
                        <a class="delete" onclick="javascript:return confirm('<?php echo osc_esc_js(__('This action can not be undone. Are you sure you want to continue?', 'passion')); ?>')" href="<?php echo osc_item_delete_url();?>" ><?php _e('Delete', 'passion'); ?></a>
                        <?php if(osc_item_is_inactive()) {?>
                        <span>|</span>
                        <a href="<?php echo osc_item_activate_url();?>" ><?php _e('Activate', 'passion'); ?></a>
                        <?php } ?>
                    </span>
                <?php } ?>
            </div>
