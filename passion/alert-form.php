...
$(document).ready(function(){
    $(".sub_button").click(function(){
        $.post('<?php echo osc_base_url(true); ?>', {email:$("#alert_email").val(), userid:$("#alert_userId").val(), alert:$("#alert").val(), page:"ajax", action:"alerts"},
            function(data){
                if(data==1) { alert('<?php echo osc_esc_js(__('You have sucessfully subscribed to the alert', 'passion')); ?>'); }
                else if(data==-1) { alert('<?php echo osc_esc_js(__('Invalid email address', 'passion')); ?>'); }
                else { alert('<?php echo osc_esc_js(__('There was a problem with the alert', 'passion')); ?>');
                };
        });
        return false;
    });
...
<div class="alert_form">
    <?php if(function_exists('osc_search_alert_subscribed') && osc_search_alert_subscribed()) { ?>
        <h3>
            <strong><?php _e('Already subscribed to this search', 'passion'); ?></strong>
        </h3>
    <?php } else { ?>
        <form action="<?php echo osc_base_url(true); ?>" method="post" name="sub_alert" id="sub_alert" class="nocsrf">
                ...
                <?php }; ?>
                <button type="submit" class="sub_button" ><?php _e('Subscribe now', 'passion'); ?>!</button>
        </form>
    <?php } ?>
</div>